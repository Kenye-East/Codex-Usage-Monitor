from usage_overlay.config import AppConfig, CachedUsageSnapshot, ConfigStore, resolve_language
...
def test_config_persists_a_verified_usage_snapshot(tmp_path):
    store = ConfigStore(tmp_path / "config.json")
    snapshot = CachedUsageSnapshot("2026-07-16T01:00:00Z", None, None, 23, 1784718617)

    store.save(AppConfig(usage_snapshot=snapshot))

    assert store.load().usage_snapshot == snapshot
