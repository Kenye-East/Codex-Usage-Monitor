...
from usage_overlay.providers.codex import CodexProvider, latest_rate_limits_from_file
...
def test_fetch_does_not_restore_a_missing_window_from_an_older_snapshot(tmp_path):
    ...
    sessions.mkdir()
    (sessions / "older.jsonl").write_text(json.dumps({"payload": {"rate_limits": {
        "primary": {"used_percent": 18, "window_minutes": 300, "resets_at": 100},
    }}}) + "\n", encoding="utf-8")
    newest = sessions / "newest.jsonl"
    newest.write_text(json.dumps({"payload": {"rate_limits": {
        "primary": {"used_percent": 49, "window_minutes": 10080, "resets_at": 200},
    }}}) + "\n", encoding="utf-8")
    # File mtime is the ordering used by the provider.
    newest.touch()
